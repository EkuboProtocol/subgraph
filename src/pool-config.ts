...
export function parseFee(config: Bytes): BigInt {
  return BigInt.fromUnsignedBytes(Bytes.fromUint8Array(config.subarray(FEE_OFFSET, TYPE_CONFIG_OFFSET).reverse()))
}

export function parseTypeConfig(config: Bytes): u32 {
  return Bytes.fromUint8Array(config.subarray(TYPE_CONFIG_OFFSET).reverse()).toU32()
}
